...
def searchdata(numContrato):
    cursor.execute('SELECT [numeroContrato],[titularContrato],[StatusContrato],[nombrePlan],[idAfiliado] FROM [dbo].[ContratoSeguroSalud] INNER JOIN [dbo].[PlanSalud] ON ([dbo].[ContratoSeguroSalud].[idPlan] = [dbo].[PlanSalud].[idPlan]) WHERE [numeroContrato] = ' + numContrato)
    consulta = cursor.fetchall()
    return consulta
